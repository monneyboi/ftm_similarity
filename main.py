...
def create_composite_properties(properties: Dict[str, Any]) -> Dict[str, Any]:
...
def extract_entity_properties(entity: Dict[str, Any]) -> Dict[str, str]:
    ...
    for prop in TARGET_PROPERTIES:
        if prop in enhanced_properties:
            processed_value = preprocess_property(enhanced_properties[prop])
            if processed_value:
...
def embed_properties(
    properties: Dict[str, str], model: SentenceTransformer
) -> Dict[str, np.ndarray]:
    ...
    for prop in common_properties:
        if prop in weights:
            similarity = cosine_similarity(
                query_embeddings[prop], candidate_embeddings[prop]
            )
            weight = weights[prop]
...
def embed(json_file: Path, model: str, output_dir: str):
    ...
    for prop, count in sorted(
        property_counts.items(), key=lambda x: x[1], reverse=True
    ):
...
def search(person_id: str, embeddings_dir: str, top_k: int):
    ...
    try:
        ...
        with click.progressbar(
            embeddings.items(), label="Calculating similarities"
        ) as bar:
        ...
        click.echo(
            f"Description: {query_props.get('description', 'N/A')[:100]}{'...' if len(query_props.get('description', '')) > 100 else ''}"
        )
        ...
        for i, result in enumerate(similarities[:top_k], 1):
            ...
            click.echo(
                f"   Description: {candidate_props.get('description', 'N/A')[:100]}{'...' if len(candidate_props.get('description', '')) > 100 else ''}"
            )
            click.echo(f"   Nationality: {candidate_props.get('nationality', 'N/A')}")

            # Always show property similarities and weighted average calculation
            click.echo("   Property Similarities:")
            prop_sims = result["property_similarities"]
            total_weighted_score = 0.0
            total_weight_used = 0.0

            for prop, sim in sorted(
                prop_sims.items(), key=lambda x: x[1], reverse=True
            ):
                weight = weights.get(prop, 0.0)
                weighted_contribution = weight * sim
                total_weighted_score += weighted_contribution
                total_weight_used += weight
                click.echo(
                    f"     {prop}: {sim:.4f} (weight: {weight:.3f}, contribution: {weighted_contribution:.4f})"
                )

            calculated_weighted_avg = (
                total_weighted_score / total_weight_used
                if total_weight_used > 0
                else 0.0
            )
            click.echo(
                f"   Weighted Average: {calculated_weighted_avg:.4f} (total weight used: {total_weight_used:.3f})"
            )
